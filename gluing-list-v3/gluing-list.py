...
def glue_selected_files(input_dir, selected_files, all_files):
    ...
    with open(output_path, 'w', encoding='utf-8') as out_file:
        # Добавляем список всех файлов в начале отчета
        out_file.write("=== СПИСОК ВСЕХ ФАЙЛОВ В ПАПКЕ ===\n")
        for i, file_path in enumerate(all_files, 1):
            out_file.write(f"{i}. {file_path}\n")
        out_file.write("\n" + "=" * 50 + "\n\n")
        
        # Добавляем информацию о выбранных файлах
        out_file.write(f"=== ВЫБРАНО ФАЙЛОВ: {len(selected_files)} ИЗ {len(all_files)} ===\n\n")
...
if __name__ == "__main__":
    ...
    glue_selected_files(input_dir, selected_files, all_files)